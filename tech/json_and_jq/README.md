...
```
The field `c` obviosly contains JSON, but how to decode it?
...
`map(. as $row | $cols | map($row[.])) as $rows` takes all objects in the outer array, and iterates through all the object keys (key1 and key2). It appends the values to an array, which gives you an array of arrays with the values, which is what you want when you're transforming JSON into CSV.
...
"key1value3","key2value3"
"key1value4","key2value4"
"key1value5","key2value5"
```
