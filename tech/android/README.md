# De-googling standard Android

First of, do not login or activate Google Play. (Unless you need to)

Then use the link below to get apps. Most things can be gotten through F-droid.

[Get apps](get_apps/)

Install Fossify alternatives to phone, text, contacts and starter app.

Then start removing and deactivating as much Google crap as you can live with.

Some caveats:
- Disabling Google Framework Services is definitely a possibility, but:
  - Lot's of apps use it and wont work without it.
  - Some apps have workarounds that can work.
  - So check each app before doing that.
- It seems that if `Google Play Store` is deactivated on the main profile, it wont work on other profiles.
  - Installs will be stuck in "Waiting" for ever.
...
If the devices has multiple users, the `pm ... --user 0 ...` commands above will only work for the primary admin user.

To disable or uninstall apps for other users, find the user id by running:
```
adb shell dumpsys user
```

It should return a lot of user info starting with:
```
Current user: 0

Users:
  UserInfo{0:<user name>:xxx} serialNo=0 isPrimary=true

    ... a lot more ...

  UserInfo{10:<some other user name>:xxx} serialNo=10 isPrimary=false

    ... a lot more ...

```
In this case `10` is the number to use with `--user nn` to disable or uninstall apps for user `<some other user name>`.
